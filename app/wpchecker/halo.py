...
class Halo(object):
    def __init__(self, halo_key, halo_secret):
        ...
        self.csm_policy_module = cloudpassage.ConfigurationPolicy(self.halo_session)  # NOQA
    ...
    def server_is_a_webserver(self, server_id):
        ...
        server_software_inventory = self.scan_module.last_scan_results(server_id,  # NOQA
                                                                       "svm")
        for target_package in target_packages:
            if "scan" not in server_software_inventory:
                print("    Server with ID %s has no SVM scan information!" % server_id)  # NOQA
                continue
    ...
    def get_fim_baseline(self, policy_id, baseline_id):
        ...
        url = "/v1/fim_policies/%s/baselines/%s/details" % (policy_id, baseline_id)  # NOQA
        return hh.get(url)
